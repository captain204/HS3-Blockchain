...
def test_quickly_mined_block():
    last_block = Block.mine_block(Block.genesis(),{'name':'Mike','Provider':'Reliance Healthcare'})
    mined_block = Block.mine_block(last_block,{'name':'Dennis','Provider':'Reliance Healthcare'})
...
def test_mined_block_difficulty_Limit_at_1():
    test_data = {'name':'Dennis','Provider':'Reliance Healthcare'}
    last_block = Block(
        time.time_ns(),
        'test_last_hash',
        'test_hash',
    ...
    time.sleep(MINE_RATE/SECONDS)
    mined_block = Block.mine_block(last_block,{'name':'Mike','Provider':'Reliance Healthcare'})
...
def block(last_block):
    return Block.mine_block(last_block, {'name':'Mike','Provider':'Reliance Healthcare'})
...
def test_is_valid_block_bad_last_hash(last_block,block):
    block.last_hash = 'wrong hash'

    with pytest.raises(Exception,  match='The block last_hash must be correct'):
        Block.is_valid_block(last_block, block)

def test_is_valid_bad_proof_work(last_block,block):
    block.hash = 'ffff'
